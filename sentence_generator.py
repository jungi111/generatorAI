...
class SentenceGenerator:
    def __init__(self):
        (
            ...
            self.gpt_tokenizer,
            self.gpt_model,
        ) = self.load_sentence_data_and_model()

    def load_sentence_data_and_model(self):
        # 데이터 불러오기
        ...
        sentence_embedding = Embedding(input_dim=total_words, output_dim=64)(
            sentence_input
        )
        level_embedding = Embedding(input_dim=np.max(level_seq) + 1, output_dim=64)(
            level_input
        )
        ...
        concat_layer = Concatenate(axis=-1)(
            [sentence_embedding, level_embedding_repeated]
        )
        ...
        if train_model:
            ...
            # 학습된 모델 저장
            tf.keras.models.save_model(self.model, "sentence_generation_model.keras")
        else:
            # 저장된 모델 불러오기
            self.model = tf.keras.models.load_model("sentence_generation_model.keras")
    ...
    def get_sentence_by_level(self, step_no):
        sentences_by_level = []
        for idx, lev in enumerate(self.level):
            if int(step_no) == lev:
                sentences_by_level.append(self.sentences[idx])
    ...
    def select_seed_sentence_by_level(self, step_no):
        sentences_by_level = self.get_sentence_by_level(step_no)
        return random.choice(sentences_by_level)

    # 문장 생성 함수
    def generate_sentence(self, step_no, n_sentences):
        output_sentences = []
        ...
        available_indices = self.level_to_sentences_indices.get(step_no, [])
        if len(available_indices) == 0:
            print(
                f"난이도 '{step_no}'에 해당하는 문장이 없습니다. 대체 문장을 생성합니다."
            )
        ...
        while len(output_sentences) < n_sentences:
            ...
            seed_sentence = self.sentences[seed_sentence_index]

            output_sentence = seed_sentence
            while len(output_sentence.split()) < self.max_sequence_len - 1:
                token_list = self.tokenizer.texts_to_sequences([output_sentence])[0]
                ...
                predicted_index = np.random.choice(
                    range(len(predicted[0])), p=predicted[0]
                )
    ...
    def generate_sentences_with_gpt(self, step_no, n_sentences):
        generated_sentences = []  # 생성된 문장들을 저장할 리스트

        while len(generated_sentences) < n_sentences:
            # seed word와 난이도를 토큰화
            seed_word = self.select_seed_sentence_by_level(step_no)
            input_text = (
