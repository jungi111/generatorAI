...
if train_model:
    ...
    def generate_word(step_no, n_words):
        ...
        while len(output_words) < n_words:
            ...
            if re.match("^[a-zA-Z]+$", output_word) and len(output_word) > 1 and output_word not in output_words:
...
def generate_word_with_gpt(step_no, n_words, gpt_model, gpt_tokenizer):
    ...
    while generated_count < n_words:
        ...
        output = gpt_model.generate(
            input_ids,
            max_length=len(seed_word) + n_words * 2,  # seed word 길이와 생성할 단어의 최대 길이 설정
        ...
        generated_words.update(word for word in decoded_output.split() if re.match("^[a-zA-Z]{2,}$", word.lower()))
